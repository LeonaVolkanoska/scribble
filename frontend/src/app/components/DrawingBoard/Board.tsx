...
export default function Board({ name, player, socket, isMaster }: { name: string, player: { id: number; name: string, master: number } | null; socket: WebSocket | null; gameIsStarted: boolean; isMaster: boolean }) {
    ...
    const WinningPromptRef = useRef<HTMLDivElement | null>(null);
    ...
    useEffect(() => {
        ...
        const handleMessage = (e: MessageEvent) => {
            ...
            if (parsedData.action === "draw") {
                ...
                setChatMessage(prevMess => [...prevMess, { playerName: parsedData.playerName, word: parsedData.word, isCorrect: parsedData.isCorrect }]);
            ...
            // }else if(parsedData.action === "generate_congratulations_message"){
            //     setWinnerName(parsedData.winnerName);
            //     setHasWon(true);
            // }
        };
    ...
    //     if (socket && socket.readyState === WebSocket.OPEN) {
    //         socket.send(JSON.stringify({
    //             action: "generate_congratulations_message",
    //             winnerName: name
    //         }))
    //     }
    // }
    ...
    const handleBoardStartDraw = (e: React.MouseEvent<HTMLCanvasElement>, isMaster: boolean) => {
        if (!isMaster) return;
        setIsDrawing(true);
    ...
    const handleBoardDraw = (e: React.MouseEvent<HTMLCanvasElement>, isMaster: boolean) => {
        if (!isMaster) return;
        if (!isDrawing) return;
    ...
    const handleBrushColorChange = (e: React.ChangeEvent<HTMLInputElement>, isMaster: boolean) => {
        if (!isMaster) return;
        setColor(e.target.value);
    };

    const handleCanvasColorChange = (e: React.ChangeEvent<HTMLInputElement>, isMaster: boolean) => {
        if (!isMaster) return;
        setBoardColor(e.target.value);
        socket?.send(JSON.stringify({ action: "board_color_change", boardColor: e.target.value }))
    };
    ...
    return (
        <div className={styles.container}>
            <div>
                ...
                <WordGenerator isMaster={isMaster} setPickedWord={setPickedWord} sendGeneratedWord={sendGeneratedWord} pickedWord={pickedWord} />

                {hasWon && <WinningPrompt  winnerName={winnerName} ref={WinningPromptRef} handleRestartGame={() => handleRestartGame(hasWon)} />}
                    
            </div>
            <div className={styles["palette-container"]}>
                <Palette initialColor={boardColor} title='canvas background' onColorChange={(e) => handleCanvasColorChange(e, isMaster)} />
                <Palette title='brush color' onColorChange={(e) => handleBrushColorChange(e, isMaster)} />
            </div>
            ...
            <div className={styles.board}>
                <canvas
                    ...
                    id="board"
                ></canvas>
                <Chat isMaster={isMaster} playerName={name} chatMessage={chatMessage} handleChatMessage={chat} check={checkIfCorrect} />
            </div>
        </div>
    );
}
...
// - timer of 2 mins ***
// win / lose prompt(done) with name
